...
class DeepFusionGAN:
    ...
    def fit(self, train_loader: DataLoader, num_epochs: int = 310) -> Tuple[List[float], List[float], List[float]]:
        ...
        for epoch in trange(num_epochs, desc="Train Deep Fusion GAN"):
            ...
            print(f'g_loss: {np.mean(g_losses)}, d_loss: {np.mean(d_losses)}, d_gp_loss: {np.mean(d_gp_losses)}')
            if (epoch + 1) % 10 == 0:
